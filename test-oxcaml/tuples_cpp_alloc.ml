...
(* replicate the alloc test, but now for CPP... *)
(*$ Ppx_c_bindings_test_oxcaml_cinaps_helpers.include_file ~file:"tuples_alloc.ml" [] *)
(* just copy the no-alloc tests but do use alloc versions instead ... *)
...
let%expect_test "unboxed tuple return: #( int value * int value )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ Val_int(63005), Val_int(63005) }); |} : #(int value * int value))]
  in
...
let%expect_test "unboxed tuple return: #( int value * Ox.u64 )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ Val_int(63005), 64013 }); |} : #(int value * Ox.u64))]
  in
...
let%expect_test "unboxed tuple return: #( int value * Ox.i64 )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ Val_int(63005), -64014 }); |} : #(int value * Ox.i64))]
  in
...
let%expect_test "unboxed tuple return: #( int value * Ox.f64 )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ Val_int(63005), 64.016 }); |} : #(int value * Ox.f64))]
  in
...
let%expect_test "unboxed tuple return: #( int value * Ox.isize )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ Val_int(63005), -64017 }); |} : #(int value * Ox.isize))]
  in
...
let%expect_test "unboxed tuple return: #( int value * Ox.mem )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ Val_int(63005), ( void* )0x64018 }); |} : #(int value * Ox.mem))]
  in
...
let%expect_test "unboxed tuple return: #( int value * int Ox.Ptr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ Val_int(63005), ( void* )0x64019 }); |}
       : #(int value * int Ox.Ptr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( int value * int Ox.Addr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ Val_int(63005), ( void* )0x64021 }); |}
       : #(int value * int Ox.Addr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.u64 * int value )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ 64013, Val_int(63005) }); |} : #(Ox.u64 * int value))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.u64 * Ox.mem )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ 64013, ( void* )0x64018 }); |} : #(Ox.u64 * Ox.mem))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.u64 * int Ox.Ptr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ 64013, ( void* )0x64019 }); |} : #(Ox.u64 * int Ox.Ptr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.u64 * int Ox.Addr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ 64013, ( void* )0x64021 }); |} : #(Ox.u64 * int Ox.Addr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.i64 * int value )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ -64014, Val_int(63005) }); |} : #(Ox.i64 * int value))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.i64 * Ox.mem )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ -64014, ( void* )0x64018 }); |} : #(Ox.i64 * Ox.mem))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.i64 * int Ox.Ptr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ -64014, ( void* )0x64019 }); |} : #(Ox.i64 * int Ox.Ptr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.i64 * int Ox.Addr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ -64014, ( void* )0x64021 }); |} : #(Ox.i64 * int Ox.Addr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.f64 * int value )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ 64.016, Val_int(63005) }); |} : #(Ox.f64 * int value))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.f64 * Ox.mem )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ 64.016, ( void* )0x64018 }); |} : #(Ox.f64 * Ox.mem))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.f64 * int Ox.Ptr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ 64.016, ( void* )0x64019 }); |} : #(Ox.f64 * int Ox.Ptr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.f64 * int Ox.Addr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ 64.016, ( void* )0x64021 }); |} : #(Ox.f64 * int Ox.Addr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.isize * int value )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ -64017, Val_int(63005) }); |} : #(Ox.isize * int value))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.isize * Ox.isize )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ -64017, -64017 }); |} : #(Ox.isize * Ox.isize))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.isize * Ox.mem )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ -64017, ( void* )0x64018 }); |} : #(Ox.isize * Ox.mem))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.isize * int Ox.Ptr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ -64017, ( void* )0x64019 }); |} : #(Ox.isize * int Ox.Ptr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.isize * int Ox.Addr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ -64017, ( void* )0x64021 }); |} : #(Ox.isize * int Ox.Addr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.mem * int value )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64018, Val_int(63005) }); |} : #(Ox.mem * int value))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.mem * Ox.u64 )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ ( void* )0x64018, 64013 }); |} : #(Ox.mem * Ox.u64))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.mem * Ox.i64 )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ ( void* )0x64018, -64014 }); |} : #(Ox.mem * Ox.i64))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.mem * Ox.f64 )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ ( void* )0x64018, 64.016 }); |} : #(Ox.mem * Ox.f64))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.mem * Ox.isize )" =
  let tuple =
    [%c.alloc ({| CAMLreturn({ ( void* )0x64018, -64017 }); |} : #(Ox.mem * Ox.isize))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.mem * Ox.mem )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64018, ( void* )0x64018 }); |} : #(Ox.mem * Ox.mem))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.mem * int Ox.Ptr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64018, ( void* )0x64019 }); |}
       : #(Ox.mem * int Ox.Ptr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( Ox.mem * int Ox.Addr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64018, ( void* )0x64021 }); |}
       : #(Ox.mem * int Ox.Addr.Ext.t))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Ptr.Ext.t * int value )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64019, Val_int(63005) }); |}
       : #(int Ox.Ptr.Ext.t * int value))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Ptr.Ext.t * Ox.u64 )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64019, 64013 }); |} : #(int Ox.Ptr.Ext.t * Ox.u64))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Ptr.Ext.t * Ox.i64 )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64019, -64014 }); |} : #(int Ox.Ptr.Ext.t * Ox.i64))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Ptr.Ext.t * Ox.f64 )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64019, 64.016 }); |} : #(int Ox.Ptr.Ext.t * Ox.f64))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Ptr.Ext.t * Ox.isize )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64019, -64017 }); |} : #(int Ox.Ptr.Ext.t * Ox.isize))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Ptr.Ext.t * Ox.mem )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64019, ( void* )0x64018 }); |}
       : #(int Ox.Ptr.Ext.t * Ox.mem))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Ptr.Ext.t * int Ox.Ptr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64019, ( void* )0x64019 }); |}
...
let%expect_test "unboxed tuple return: #( int Ox.Ptr.Ext.t * int Ox.Addr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64019, ( void* )0x64021 }); |}
...
let%expect_test "unboxed tuple return: #( int Ox.Addr.Ext.t * int value )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64021, Val_int(63005) }); |}
       : #(int Ox.Addr.Ext.t * int value))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Addr.Ext.t * Ox.u64 )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64021, 64013 }); |} : #(int Ox.Addr.Ext.t * Ox.u64))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Addr.Ext.t * Ox.i64 )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64021, -64014 }); |} : #(int Ox.Addr.Ext.t * Ox.i64))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Addr.Ext.t * Ox.f64 )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64021, 64.016 }); |} : #(int Ox.Addr.Ext.t * Ox.f64))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Addr.Ext.t * Ox.isize )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64021, -64017 }); |} : #(int Ox.Addr.Ext.t * Ox.isize))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Addr.Ext.t * Ox.mem )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64021, ( void* )0x64018 }); |}
       : #(int Ox.Addr.Ext.t * Ox.mem))]
  in
...
let%expect_test "unboxed tuple return: #( int Ox.Addr.Ext.t * int Ox.Ptr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64021, ( void* )0x64019 }); |}
...
let%expect_test "unboxed tuple return: #( int Ox.Addr.Ext.t * int Ox.Addr.Ext.t )" =
  let tuple =
    [%c.alloc
      ({| CAMLreturn({ ( void* )0x64021, ( void* )0x64021 }); |}
...
let%expect_test "unboxed local tuples" =
  let a : string @ local = "A" in
  let b : string @ local = "B" in
  let #(c, d) : #(string * string) @ local =
    [%c.alloc
      ({| CAMLreturn({ %{a:string local_value}, %{b:string local_value} }); |}
  ...
  [%expect {| (A B) |}];
  let #(e, f) : #(Ox.i64 * string) @ local =
    [%c.alloc
      ({| CAMLreturn({ 42, %{b:string local_value} }); |}
       : #(Ox.i64 * string local_value))]
  in
  assert (phys_equal b f);
  print_endline [%string {| (%{e#Ox.I64} %{f}) |}];
  [%expect {| (42 B) |}];
  let #(g, h) : #(string * Ox.i64) @ local =
    [%c.alloc
      ({| CAMLreturn({ %{a:string local_value}, 42 }); |}
       : #(string local_value * Ox.i64))]
  in
