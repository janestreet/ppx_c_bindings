...
let%expect_test "unboxed tuple return: #( Ox.isize * Ox.isize )" =
  let tuple = [%c.alloc ({| CAMLreturn({ -64017, -64017 }); |} : Ox.isize * Ox.isize)] in
...
let%expect_test "unboxed local tuples" =
  ...
  let (e, f) : Ox.i64 * string =
    [%c.alloc
      ({| CAMLreturn({ 42, %{b:string local_value} }); |} : Ox.i64 * string local_value)]
  in
  ...
  let (g, h) : string * Ox.i64 =
    [%c.alloc
      ({| CAMLreturn({ %{a:string local_value}, 42 }); |} : string local_value * Ox.i64)]
  in
